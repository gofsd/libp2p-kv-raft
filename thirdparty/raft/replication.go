...
// getPeer returns the peer under peerLock. LOCAL PATCH (see thirdparty's
// entry in this repo's README): 'peer' is written by startStopReplication
// whenever a follower's address changes, so every read of it has to hold
// peerLock -- most already did so inline, but updateLastAppended and two
// error-path log statements read it unguarded, which is a genuine data
// race, not just a test artifact. It fires readily here because a peer's
// address really does change under a live leader in this project (a node
// that swaps a direct address for a /p2p-circuit relay one).
func (s *followerReplication) getPeer() Server {
	s.peerLock.RLock()
	peer := s.peer
	s.peerLock.RUnlock()
	return peer
...
func (r *Raft) pipelineSend(s *followerReplication, p AppendPipeline, nextIdx *uint64, lastIndex uint64) (shouldStop bool) {
	...
	if _, err := p.AppendEntries(req, new(AppendEntriesResponse)); err != nil {
		r.logger.Error("failed to pipeline appendEntries", "peer", s.getPeer(), "error", err)
		return true
	}
...
func (r *Raft) handleStaleTerm(s *followerReplication) {
	r.logger.Error("peer has newer term, stopping replication", "peer", s.getPeer())
...
func updateLastAppended(s *followerReplication, req *AppendEntriesRequest) {
	...
	if logs := req.Entries; len(logs) > 0 {
		...
		s.commitment.match(s.getPeer().ID, last.Index)
	}
