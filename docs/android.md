...
(`MainActivity.kt`, `CommandCatalog.kt`, `CommandDetailActivity.kt`) is enough context — you don't
...
- `android-app/` — the actual reference implementation: `MainActivity.kt` (bring-up),
  `CommandCatalog.kt` (every call, one `CommandSpec` each), `CommandDetailActivity.kt` (the
  off-thread call pattern), `OutputLog.kt` (fan-in for the three standing-subscription callbacks).
