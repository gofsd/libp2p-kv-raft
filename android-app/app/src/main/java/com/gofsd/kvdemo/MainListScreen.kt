...
/**
 * The app's start screen: a pull-to-refresh list of exactly two entries, "Commands" and "Groups"
 * -- not live data, just this app's own two navigation-shortcut generators, but pull-to-refresh
 * is still wired up for real (a brief spinner, then the same two rows again) since that gesture,
 * not a static menu, is the interaction this screen is modeled on.
 *
 ...
 * category's command list when scanned). Neither screen executes anything -- see NavCode's own
 * doc comment for why these two codes are pure navigation, distinct from CommandDetailScreen's
