...
func main() {
	...
	relayMaxCircuitsPerPeer := flag.Int("relay-max-circuits-per-peer", 0, "only used alongside -relay-service: concurrent open relayed circuits a single peer may hold through this node (0 = shmevent.DefaultRelayMaxCircuitsPerPeer, 16)")
	...
	relayMaxReservationsPerIP := flag.Int("relay-max-reservations-per-ip", 0, "only used alongside -relay-service: active relay-slot reservations allowed from one IP address (0 = shmevent.DefaultRelayMaxReservationsPerIP, 64)")
	...
	quotaRelayEventsPerPeerPerSec := flag.Float64("quota-relay-events-per-peer-per-sec", 0, "sustained rate of relay-slot reservations/connects allowed from one remote peer id (0 = daemon.DefaultQuotaRelayEventsPerPeerPerSec, 32/sec); over budget is denied at the same point as failing the relay group-ACL")
	quotaRelayBurstPerPeer := flag.Int("quota-relay-burst-per-peer", 0, "instantaneous event burst allowed on top of -quota-relay-events-per-peer-per-sec (0 = daemon.DefaultQuotaRelayBurstPerPeer, 128)")
	quotaRelayEventsPerIPPerSec := flag.Float64("quota-relay-events-per-ip-per-sec", 0, "sustained rate of relay-slot reservations/connects allowed from one remote IP address (0 = daemon.DefaultQuotaRelayEventsPerIPPerSec, 128/sec)")
	quotaRelayBurstPerIP := flag.Int("quota-relay-burst-per-ip", 0, "instantaneous event burst allowed on top of -quota-relay-events-per-ip-per-sec (0 = daemon.DefaultQuotaRelayBurstPerIP, 512)")
