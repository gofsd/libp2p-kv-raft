...
// reads the same bytes back off disk afterward. That makes ordering
// harmless: pkg/kvctl's bootUp already has the daemon write ready.json
// (Config.ReadyFileName) before any client's waitForReady returns and
// issues its first call, so in practice the daemon always wins the race,
...
func loadOrGenerateToken(dataDir string) (string, error) {
	...
	if err := os.WriteFile(path, []byte(tok), 0o600); err != nil {
		return "", fmt.Errorf("ipc: write token file %s: %w", path, err)
	}
	return tok, nil
}
